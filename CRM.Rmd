...
---
![Une vie de babouine](ncomms11181-f1.jpg)


Note^[tiré de @tung_cumulative_2016, les deux courbes represente la survie de babouine exposé à 1, 2 ou plus conditions adverses]
...
* Des individus i qui possèdent des cartaristiques Xi à travers le temps
...
* Des agrégations qui enrichissent le fichier des individus de caratères comportementale dérivée de la séquence des achat, en particulier  la durée écoulée entre le premier achat et le dernier.
...
Purchase<- read.csv2("~/AtelierR/botanic/EXTRACTION_TICKET.csv")
Purchase<-Purchase %>% rename(Montant=tic_totalTTC,Points=tic_pointObtenu,Date=tic_date, magasin=mag_code) 
```
...
Customer<- Customer[c(-5,-6,-7,-8,-16,-17,-18,-19,-20,-21,-22)]

Customer$idclient<-as.factor(Customer$idclient)
Purchase$idclient<-as.factor(Purchase$idclient)
Purchase$Montant<-as.numeric(Purchase$Montant)

#merge of the customer and c1_pf.
Customer<-subset(Customer,echantillon1==1)
Purchase<-merge(Purchase,Customer, by.x="idclient",by.y="idclient", all.x=FALSE, all.y=FALSE, sort=TRUE)
Purchase<-Purchase[c(-7,-8)]
```

# Description de la série

Pour obtenir un sens plus concrêt des données en voici une représentation simple : le nombre de transactions journalières effectuées par les clients qui ont adopté la carte en 2010 au cours des quatre années suivantes. On y observe :
...
Dans la séquence suivante on spécifie le modèle, puis avec la fonction `cox.zph` on evalue la proportionnalité des risques. Les deux denières lignes sont destinées à représenter les courbes de survie et de risque cumulé pour une variable qui attire notre intention. C'est une reprensation stratifiée


```{r duration04, caption ="Méthode de Cox"}

#cox
res.cox1 <- coxph(SurvObj ~  C_sms + C_mail + C_email+HABITE+ANIMAUX_chien, data =  c1)
res.cox1
...
analyser les résultats : on s'apercoit qu'assi bien le cumul des achats que l'evenement achat tendent à 
...
c1_pf<-merge(c1_f,Purchase[,1:8],by="idclient", all=FALSE)
...
On sait donc introduire dans un modèle de survie, le modèle de Cox, deux types de variables, des variables invariantes au cours du temps qui exercent un effet proportionnel constant au cours tu temps, et d'autres donc la valeur variant au cours du temps peuvent affecter de manière différentielle les espérances de survie.
...
#cox
res.cox1 <- coxph(SurvObj ~  HABITE+ANIMAUX_chien+ C_sms +C_email + C_mail +tt(C_mail), data =  c1, tt = function(x, t, ...) x * log(t+1))
res.cox1
#plot(res.zph1)
```
...
M3<-lme(total_1_L~ptscum+Points_l+Points_l*rank_l+Points_l,random=~1|magasin/idclient/rank,data=pts_ln)
summary(M3)

stargazer(M1,M2,M3, type="text")
```

On utilise une fonction de `Ape` pour décomposer les élements de la variance. Dans notre cas la variabilité des individus représente 9.7% de la variance de la composante alétoire du modèle, le rang de l'achat pour 75,3%, seulement 0.7% pour les magasin ce qui indique une grande homogénéité geographique,  et 14,2% doivent être attribué à des causes autres, propres à chaque achat.
...
En dépit du pouvoir très faiblement explicatif de nos variables effets
...
On laisse le soin au lecteur d'ajuster le code précédent à l'étude des durées inter-achat.


#Conclusion

Deux outils complémentaires pour analyser la dynamique des ventes d'un segment adhérant à un programme de fidélisation à deux niveau d'analyse :

 * le niveau des transactions : en classant selon l'ordre des achats consruire des modèle linéaire à variable retardée ( on explitique t+1 avec t), et en particilier des modèles linéaires hieracharqui pour décomposer la variance résiduelles dans ses différentes sources hypothétoique : la géographie avec le poitn de vente, l'hétérogénéité individuelle ( les individus), et même le rang de l'achat qui indique au moins une expérience grandissant avec le temps et les expériences de la marque.


 * le niveau de la relation : au niveau de la relation : comparaison entre des strates, efet statique (systématique au cours du temps) des variables, modélisation de l'effet de variables qui évoluent au cours du temps, en renant en compte des effets instantanés et des effets cumulés, enfin la modélisation de paramètres qui varient dans le temps. 
